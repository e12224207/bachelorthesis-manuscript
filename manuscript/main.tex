...
graphic of most ntable families/paradigms (procedural should be identifieable)
...
\marginpar{POTL is a temporal logic specifically designed to express properties over OPLs. Extending LTL, it includes modalities that refer to the hierarchical structure induced by procedure calls and returns. This enables formal reasoning about context-sensitive behaviors, such as matching call-return sequences, stack inspection, and structured exception handling.}
\\

MiniProb extends MiniProc by incorporating primitives from PPLs \cite{POPACheck}, which enable sampling, conditioning and nested inference (see: \ref{sec:pp}). While remaining recursive-procedural, the
classification changes with it now belonging to the family of probabilistic programming languages, still within the context-free class.
...
allowing to check whether properties hold with certainty, compute exact satisfaction probabilities, or obtain estimated bounds when exact computation is infeasible \cite{guideMiniProb}.
...
MiniProb’s syntax is carefully designed to comply with the constraints of \textbf{Operator Precedence Languages (OPLs)}, ensuring compatibility with \textbf{Operator Precedence Automata (OPAs)} and their probabilistic extensions. To achieve this, the language avoids syntactic ambiguity and enforces structured, well-bracketed constructs that can be parsed deterministically using an \textbf{Operator Precedence Matrix (OPM)}. Within these constraints, MiniProb adopts a \textit{procedural and recursive} programming model, supporting user-defined functions with call-return semantics, local variable declarations, and block-structured control flow. Its syntax includes familiar C-like constructs such as \code{if}/\code{else}, \code{while}, and \code{try}/\code{catch}, alongside typed variables (e.g., \code{bool}, \code{s8}, \code{u16}, including fixed-size arrays). Extending this foundation, MiniProb incorporates core primitives from \textit{probabilistic programming languages}: \code{observe} statements condition execution on logical expressions (with trace rejection semantics), \code{query} triggers inference over program executions, and distributions such as \code{Bernoulli} and \code{Uniform} enable stochastic behavior. Together, these features allow MiniProb programs to be translated into \textbf{probabilistic OPAs (pOPAs)}, supporting formal verification and probabilistic reasoning over recursive computations.
