...
Although this extension is implemented using \textbf{Langium}, which itself builds upon the standardized \textbf{Language Server Protocol (LSP)}~\cite{LSP},
...
In contrast, more comprehensive technologies such as \textbf{ANTLR}~\cite{ANTLR} and \textbf{Langium} (built upon the \textbf{Chevrotain} parser~\cite{Chevrotain}) go beyond
...
Each approach has its benefits and trade-offs, and the choice depends upon the specific requirements, constraints, and long-term maintainability of the language tooling in question.
